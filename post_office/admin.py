...
# URI schemes are case-insensitive. The lookbehind rejects ``cid:`` inside a longer word (``acid:``);
# the Content-ID ends at whitespace, a quote, a tag delimiter or a CSS ``url(...)`` parenthesis.
CID_PATTERN = re.compile(r'(?<![A-Za-z0-9+.-])cid:([^\s"\'<>()]+)', re.IGNORECASE | re.ASCII)
...
CID_MD5_PATTERN = re.compile(r'(?<![A-Za-z0-9+.-])(?i:cid):([0-9a-f]{32})(?=$|[\s"\'<>()])', re.ASCII)
