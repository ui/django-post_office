...
# Same, restricted to the 32-hex md5 Content-IDs generated by the ``inline_image`` template tag,
# which the ``post_office_email_image`` URL route accepts. The lookahead avoids rewriting a prefix
# of a longer Content-ID.
CID_MD5_PATTERN = re.compile(r'(?<![A-Za-z0-9+.-])cid:([0-9a-f]{32})(?=$|[\s"\'<>()])', re.IGNORECASE | re.ASCII)
